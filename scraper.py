"""
Dallas Property Scraper — Step 1 of 2
---------------------------------------
Uses ATTOM's /allevents/detail endpoint which returns everything
in a single bulk call per ZIP page:

  - Full property characteristics (beds, baths, sqft, etc.)
  - Sale price + sale date
  - AVM (automated valuation model) — current estimated value
  - Full tax + assessment data (market values, tax amount, etc.)

Run this first, then run enrich.py to add crime, disaster,
and school district data.

    5%  of 500 =  25 calls ~  2,500 properties  (~30 seconds)
   10%  of 500 =  50 calls ~  5,000 properties  (~1 minute)
   25%  of 500 = 125 calls ~ 12,500 properties  (~3 minutes)
   50%  of 500 = 250 calls ~ 25,000 properties  (~6 minutes)
  100%  of 500 = 500 calls ~ 50,000 properties  (~11 minutes)

Setup:  pip install requests
Run:    python scraper.py
Then:   python enrich.py
"""
...
DAILY_LIMIT_PCT = 1   # 0.10 = 10% = 50 calls. Set to 1.0 for full runs.

OUTPUT_FILE   = "PART22_NEW_Dallas_Properties.csv"
PROGRESS_FILE = "New_Progress.json"
...
ATTOM_DAILY_CAP = 1000
PAGE_SIZE       = 100
DELAY           = 0.35

# /allevents/detail returns property info + sale + AVM + tax all in one call
...
class Keys:
    def __init__(self, raw_keys, progress):
        ...
        if not self.keys:
            raise ValueError("No API keys found! Replace YOUR_KEY_1 with your actual key.")
...
def call_attom(key, zip_code, page):
    try:
        ...
        log.warning(f"  Request failed ZIP {zip_code} p{page}: {e}")
        return None, "error"


# ---------------------------------------------------------------------------
# Flatten a property from /allevents/detail response into one CSV row.
# Field paths verified against real response data.
# ---------------------------------------------------------------------------
...
def flatten(prop):
    ...
    # AVM — current estimated market value (unique to /allevents/detail)
    avm    = prop.get("avm",         {}) or {}
    avmamt = avm.get("amount",       {}) or {}
    avmchg = avm.get("AVMChange",    {}) or {}
    avmcalc= avm.get("calculations", {}) or {}
    ...
    mkt    = asmt.get("market",      {})
    tax    = asmt.get("tax",         {})
    calcs  = asmt.get("calculations",{})

    # Owner
    owner  = prop.get("owner",       {}) or {}
    owner1 = owner.get("owner1",     {}) or {}

    # geoIdV4 codes — saved for enrich.py
    geo    = loc.get("geoIdV4",      {}) or {}
    sb_raw = geo.get("SB", "")
    sb_id  = sb_raw.split(",")[0].strip() if sb_raw else ""
    ...
    return {
        ...
        # ── geoIdV4 codes (used by enrich.py) ────────────────────
        "geoIdV4_N2":           geo.get("N2", ""),
        "geoIdV4_N1":           geo.get("N1", ""),
        "geoIdV4_N4":           geo.get("N4", ""),
        "geoIdV4_DB":           geo.get("DB", ""),
        "geoIdV4_SB":           sb_id,
        "geoIdV4_ZI":           geo.get("ZI", ""),
        ...
        "frameType":            constr.get("frameType"),
        ...
        # ── AVM (current estimated value) ─────────────────────────
        # This is ATTOM's own price model — useful as a target variable
        # proxy for properties where salePrice is blank
        "avmValue":             avmamt.get("value"),
        "avmConfidence":        avmamt.get("scr"),       # 0-100, higher = more confident
        "avmHigh":              avmamt.get("high"),
        "avmLow":               avmamt.get("low"),
        "avmValueRange":        avmamt.get("valueRange"),
        "avmLastMonth":         avmchg.get("avmlastmonthvalue"),
        "avmChangeAmt":         avmchg.get("avmamountchange"),
        "avmChangePct":         avmchg.get("avmpercentchange"),
        "avmPerSqft":           avmcalc.get("perSizeUnit"),
        "avmDate":              avm.get("eventDate"),

        # ── Assessment (county assessed values) ───────────────────
        "assessedTotal":        asd.get("assdttlvalue"),
        "assessedLand":         asd.get("assdlandvalue"),
        "assessedImprov":       asd.get("assdimprvalue"),
        "assessedPerSqft":      asd.get("assdttlpersizeunit"),
        "assessedImprPerSqft":  asd.get("assdimprpersizeunit"),

        # ── Market values ─────────────────────────────────────────
        "marketTotal":          mkt.get("mktttlvalue"),
        "marketLand":           mkt.get("mktlandvalue"),
        "marketImprov":         mkt.get("mktimprvalue"),

        # ── Tax ───────────────────────────────────────────────────
        "taxAmount":            tax.get("taxamt"),
        "taxYear":              tax.get("taxyear"),
        "taxPerSqft":           tax.get("taxpersizeunit"),

        # ── Assessment calculations ───────────────────────────────
        "calcTotalValue":       calcs.get("calcttlvalue"),
        "calcLandValue":        calcs.get("calclandvalue"),
        "calcImprValue":        calcs.get("calcimprvalue"),
        "calcValuePerSqft":     calcs.get("calcvaluepersizeunit"),
...
def main():
    ...
    log.info("=" * 60)
    log.info("Dallas Property Scraper  (Step 1 of 2)")
    log.info("=" * 60)
    log.info(f"  Endpoint:        /allevents/detail")
    log.info(f"  Returns:         property + sale + AVM + tax in one call")
    log.info(f"  Keys:            {total_keys}")
    log.info(f"  Limit:           {DAILY_LIMIT_PCT*100:.0f}% = {CALLS_PER_KEY} calls/key")
    log.info(f"  Total calls:     {total_calls:,}")
    log.info(f"  Est. properties: ~{total_calls * PAGE_SIZE:,}")
    log.info(f"  Est. runtime:    ~{est_mins:.0f} minutes")
    log.info(f"  When done, run:  python enrich.py")
    log.info("=" * 60)
    ...
    log.info(f"{len(todo)} ZIPs remaining.\n")
    ...
    try:
        for zip_code in todo:
            ...
            while True:
                ...
                if key is None:
                    ...
                    key = keys.get()
                    if key is None:
                        log.error("No keys available after reset.")
                        return
                ...
                total_with_avm   += avm_count

                price_pct = (total_with_price / total_written * 100) if total_written else 0
                avm_pct   = (total_with_avm   / total_written * 100) if total_written else 0
                log.info(f"  Page {page}/{total_pages} — "
                         f"+{new_count} saved | "
                         f"sale price: {price_pct:.0f}% | "
                         f"AVM: {avm_pct:.0f}% | "
        ...
        log.info(f"  Sale price present: {total_with_price:,} ({price_pct:.1f}%)")
        log.info(f"  AVM present:        {total_with_avm:,} ({avm_pct:.1f}%)")
        log.info(f"  Next step: python enrich.py")
        log.info("=" * 60)
